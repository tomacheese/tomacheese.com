...
1. **最小限の変更**を心がける
1. **既存のコンポーネント構造**を理解してから変更
1. **日本語コンテンツ**の文脈を考慮
1. **レスポンシブデザイン**を維持
1. **SEO 対応**（meta tags, structured data 等）を考慮
...
1. リンティングエラーがないことを確認
1. 既存機能への影響がないことを確認
1. **検証用ファイルやテスト用ファイルを削除**
1. すべてのCIが成功するまで監視・対応する。失敗した場合は修正を行い、コミット・プッシュし、再度監視を行う。
...
1. **検証用ファイルの削除**
   ...
   find /tmp -type f \
     \( -name "test-*" -o -name "verify-*" -o -name "experiment-*" \) \
     -delete
   ```
...
1. プロジェクト全体の理解を深める
1. 変更対象のファイルとその関連ファイルを確認
1. 型定義やインターフェースの整合性を検証
1. テストファイルの存在を確認し、必要に応じてテストも更新
1. 変更の影響範囲を明確にして提案
